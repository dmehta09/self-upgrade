#!/usr/bin/env node
/* ============================================================
   Senior Backend Scenario Q&A — page assembler (dev-time only;
...
const ICON = "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%2314b8a6' stroke-width='2.2' stroke-linecap='round' stroke-linejoin='round'><rect x='2' y='3' width='20' height='8' rx='2'/><rect x='2' y='13' width='20' height='8' rx='2'/><path d='M6 7h.01M6 17h.01'/></svg>";
const BRAND = "Senior Backend Scenario Q&A";
...
function scripts(p, widgets) {
  ...
  var ws = widgets || [];
  if (ws.indexOf("drill-bank.js") >= 0) {
    s += `  <script src="${p}assets/js/sbe-bank.js"></script>\n`;
  }
  ws.forEach(function (w) { s += `  <script src="${p}assets/js/${w}"></script>\n`; });
  s += `  <script src="${p}assets/js/progress.js"></script>\n`;
  s += `  <script src="${p}assets/js/mermaid-init.js"></script>\n`;
  return s;
}
...
function stubSections(page) {
  ...
  return `          <section class="hero reveal">
            ...
            <h1>${page.title}</h1>
            <p class="lead">This page of ${BRAND} is being written &mdash; senior scenario answers are on the way.</p>
          </section>
...
<head>
  ...
  <link rel="stylesheet" href="${p}assets/css/senior-backend.css" />
...
${mainWrap(page, sections, pages, i)}
      <footer class="footer">
        <span class="fmono">${BRAND} &middot; Scenario interview Q&amp;A · FastAPI-first · Nest notes</span>
...
function main() {
  // Always refresh domain fragments + drill bank from JSON banks
  try {
    require("child_process").execFileSync(process.execPath, [path.join(__dirname, "render-banks.js")], { stdio: "inherit" });
  } catch (e) {
    console.error("render-banks.js failed:", e.message);
    process.exit(1);
  }
