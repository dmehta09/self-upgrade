#!/usr/bin/env node
/* ============================================================
   Observability Field Guide — page assembler (dev-time only; output
...
/* pulse / EKG glyph — the "signal" of observability */
const ICON = "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%2322d3ee' stroke-width='2.2' stroke-linecap='round' stroke-linejoin='round'><path d='M2 13h4l2.5 7 5-16 2.5 9H22'/></svg>";
const BRAND = "Observability Field Guide";

function sidebar(p) {
  const groups = M.sidebarGroups();
  let s = `    <aside class="sidebar" id="sidebar">\n`;
  s += `      <a class="brand" href="${p}index.html" aria-label="${BRAND} home">\n`;
  s += `        <span class="brand-mark"><svg viewBox="0 0 24 24" fill="none" stroke="#06121a" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 13h4l2.5 7 5-16 2.5 9H22"/></svg></span>\n`;
  s += `        <span class="brand-text"><b>${BRAND}</b><span>Monitoring &rarr; observability &middot; interview-ready</span></span>\n`;
  s += `      </a>\n`;
...
function stubSections(page) {
  ...
  return `          <section class="hero reveal">
            ...
            <h1>${page.title}</h1>
            <p class="lead">This page of the ${BRAND} is being written &mdash; sharp, visual, interview-ready content is on the way.</p>
          </section>
...
function renderPage(page, sections, pages, i) {
  ...
  const desc = BRAND + " — " + page.title + ". A sharp, visual guide to observability — logs, metrics, traces, OpenTelemetry, Prometheus, Grafana & SLOs — for interviews — June 2026.";
  return `<!DOCTYPE html>
...
<head>
  ...
  <link rel="stylesheet" href="${p}assets/css/observability.css" />
...
${mainWrap(page, sections, pages, i)}
      <footer class="footer">
        <span class="fmono">${BRAND} &middot; logs &middot; metrics &middot; traces &middot; OpenTelemetry &middot; interview-ready</span>
