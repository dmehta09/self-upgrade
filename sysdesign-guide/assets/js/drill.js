/* ============================================================
   System Design Field Guide — mock-interview drill engine (.drill)
   A timed 35-minute interview simulator. Pick a question from
   window.SD_DRILLS, run the five phases (requirements → estimation
   → high-level → deep dive → wrap-up) against the clock with
   interviewer-style nudges and reveal-on-demand hints, then grade
   yourself against a staff-written rubric and compare with the
   model answer. History persists to localStorage["sd-drill"].
...
(function () {
  ...
  function init(host) {
    ...
    function renderIntro() {
      ...
      head.appendChild(el("span", "dr-title", "Mock-interview drills"));
      head.appendChild(el("span", "dr-best", BANK.length + " questions &middot; 35:00 each"));
    ...
    function renderGrade(timedOut) {
      ...
      host.appendChild(head);
      var intro = el("p", "drill-doing", (timedOut ? "Time's up. " : "") + "Be honest — tick only what you actually said out loud. The rubric is what a senior interviewer listens for.");
    ...
    /* ---------- result ---------- */
    function verdict(p) {
      return p >= 85 ? "Hire signal — this is a clean senior loop." : p >= 65 ? "Strong, with gaps — reread the unticked lines."
        : p >= 40 ? "The skeleton is there; the deep dive needs work." : "Study the lesson, then rerun this drill cold.";
    }
