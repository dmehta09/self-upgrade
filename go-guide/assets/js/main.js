/* ============================================================
   A PHILOSOPHY OF SOFTWARE DESIGN — shared behaviors (no deps, offline)
   theme · mobile nav · scroll progress · scrollspy · code tabs ·
   copy · quizzes · reveal-on-scroll · Go / Python (+ shell) highlighter
   ============================================================ */
(function () {
  "use strict";
  ...
  function applyTheme(t) {
    ...
    try { localStorage.setItem("go-theme", t); } catch (e) {}
  }
  (function initTheme() {
    var saved;
    try { saved = localStorage.getItem("go-theme"); } catch (e) {}
  ...
  var PY_BI = ["print","len","str","int","float","list","dict","set","tuple","bool","range","enumerate","zip","type","isinstance","issubclass","super","self","cls","map","filter","sorted","reversed","sum","min","max","abs","input","ord","chr","divmod","pow","round","all","any","iter","next","open","format","repr","hash","id","vars","dir","getattr","setattr","hasattr","delattr",
    // OOP / design idioms — colored like builtins (APOSD code is class-heavy)
    "object","property","staticmethod","classmethod","abstractmethod","dataclass","field","Protocol","ABC","ABCMeta","Enum","IntEnum","auto","cache","cached_property","lru_cache","wraps","deepcopy","copy","namedtuple","NamedTuple","TypedDict","Self","Final","ClassVar",
    // common collections / typing names
    "deque","defaultdict","Counter","OrderedDict","collections","Optional","List","Dict","Set","Tuple","Deque","Any","Union","Callable","Iterator","Iterable","Sequence","Mapping","Node"];
  ...
  /* Go — keywords, predeclared types & builtins, strings (incl. raw `back-ticks`), runes, numbers */
  var GO_KW = ["break","case","chan","const","continue","default","defer","else","fallthrough","for","func","go","goto","if","import","interface","map","package","range","return","select","struct","switch","type","var","true","false","nil","iota"];
  var GO_BI = ["append","cap","close","complex","copy","delete","imag","len","make","new","panic","print","println","real","recover","min","max","clear",
  ...
  var GOKW = {}; GO_KW.forEach(function (k) { GOKW[k] = 1; });
  var GOBI = {}; GO_BI.forEach(function (k) { GOBI[k] = 1; });

  function highlightGo(code) {
    var re = /(\/\/[^\n]*|\/\*[\s\S]*?\*\/)|(`[^`]*`|"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*')|(\b0[xX][0-9a-fA-F_]+\b|\b\d[\d_]*\.?\d*(?:[eE][+-]?\d+)?\b)|([A-Za-z_]\w*)|([^\sA-Za-z0-9_])/g;
    var out = "", m, last = 0;
    while ((m = re.exec(code)) !== null) {
      if (m.index > last) out += esc(code.slice(last, m.index));
      last = re.lastIndex;
      if (m[1]) out += '<span class="tok-com">' + esc(m[1]) + "</span>";          // comment
      else if (m[2]) out += '<span class="tok-str">' + esc(m[2]) + "</span>";      // string / rune
      else if (m[3]) out += '<span class="tok-num">' + esc(m[3]) + "</span>";      // number
      else if (m[4]) {                                                              // identifier
        var w = m[4], after = code.charAt(re.lastIndex);
        if (GOKW[w]) out += '<span class="tok-kw">' + w + "</span>";
        else if (after === "(") out += '<span class="tok-fn">' + w + "</span>";
        else if (GOBI[w]) out += '<span class="tok-bi">' + w + "</span>";
        else out += w;
      }
      else if (m[5]) {                                                              // punctuation / operator
        var p = m[5];
        if ("|=+-*/<>:&!%^".indexOf(p) !== -1) out += '<span class="tok-op">' + esc(p) + "</span>";
        else out += esc(p);
      }
  ...
  function highlightAll() {
    document.querySelectorAll(".codeblock pre code").forEach(function (el) {
      ...
      else if (lang === "go" || lang === "golang") el.innerHTML = highlightGo(raw);
