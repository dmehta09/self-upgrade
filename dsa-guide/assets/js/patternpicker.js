...
(function () {
  ...
  var PATTERNS = {
    ...
    "monotonic-deque": { label: "Monotonic deque",         lesson: "pat-sliding-window" },
    "stack":           { label: "Stack",                   lesson: "pat-stack" },
    "monotonic-stack": { label: "Monotonic stack",         lesson: "pat-stack" },
    "prefix-sum":      { label: "Prefix sum + hash",       lesson: "pat-arrays-hashing" },
  ...
  var PATTERN_ITEMS = [
    ...
    { id: "pp-subarray-k", answer: "prefix-sum", trapKey: "sliding-window",
      blurb: "Count contiguous subarrays that sum to k. The array may contain negatives and zeros.",
      why: "Running prefix + hash of past prefixes answers \"how many earlier prefixes equal prefix−k?\" in one pass — works with negatives.",
      trap: "A shrinking sliding window assumes the sum is monotonic when you add/remove — negatives break that." },
    { id: "pp-window-max", answer: "monotonic-deque", trapKey: "heap",
      blurb: "Return the maximum of every contiguous window of width k in O(n) time.",
      why: "A decreasing deque of indices keeps the front as the window max; each index enters/leaves once.",
      trap: "A heap can track maxima but costs O(n log k) and needs stale-entry cleanup — the deque is the O(n) default." },
    { id: "pp-perms", answer: "backtracking", trapKey: "dp",
      blurb: "Return every possible ordering of a list of distinct integers.",
      why: "Generate-all orderings = DFS choose/undo with a used mask; n! leaves, not a count DP.",
      trap: "DP counts or optimizes overlapping subproblems — here the output is the full list of permutations." },
