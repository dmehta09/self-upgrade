/* ============================================================
   DSA Field Guide — coding-round drill engine (.drill)
   A timed 40-minute algorithm interview simulator. Pick a question
   from window.DSA_DRILLS, run the five phases (restate & clarify →
   brute force & Big-O → find the pattern → code it → test & edge
   cases) against the clock with interviewer-style nudges and
   reveal-on-demand hints, then grade yourself against a senior
   rubric and compare with the model answer. History persists to
   localStorage["dsa-drill"].
...
(function () {
  ...
  function init(host) {
    ...
    var PHASES = window.DSA_DRILL_PHASES || [];
    var LEVEL_ORDER = { warm: 0, core: 1, advanced: 2, expert: 3 };
    var BANK = (window.DSA_DRILLS || []).filter(function (d) {
    ...
    function renderIntro() {
      ...
      head.appendChild(el("span", "dr-title", "Coding-round drills"));
    ...
    function renderGrade(timedOut) {
      ...
      host.appendChild(head);
      var intro = el("p", "drill-doing", (timedOut ? "Time's up. " : "") + "Be honest — tick only what you actually wrote or said out loud. The rubric is what a senior interviewer checks for in a coding round.");
    ...
    /* ---------- result ---------- */
    function verdict(p) {
      return p >= 85 ? "Hire signal — named the pattern fast, coded it clean, tested it out loud." : p >= 65 ? "Strong, with gaps — reread the unticked lines, then redo this one cold."
        : p >= 40 ? "You saw the shape but the code phase slipped — rebuild the solution from memory first." : "Study the lesson, solve it untimed once, then come back to the clock.";
    }
