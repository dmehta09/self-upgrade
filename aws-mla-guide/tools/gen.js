#!/usr/bin/env node
/* ============================================================
   AWS MLA Field Guide — page assembler (dev-time only; output
...
const ICON = "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%2314b8a6' stroke-width='2.2' stroke-linecap='round' stroke-linejoin='round'><path d='M12 12c-2-2.67-4-4-6-4a4 4 0 1 0 0 8c2 0 4-1.33 6-4Zm0 0c2 2.67 4 4 6 4a4 4 0 0 0 0-8c-2 0-4 1.33-6 4Z'/></svg>";
const BRAND = "AWS MLA Field Guide";

function sidebar(p) {
  const groups = M.sidebarGroups();
  let s = `    <aside class="sidebar" id="sidebar">\n`;
  s += `      <a class="brand" href="${p}index.html" aria-label="${BRAND} home">\n`;
  s += `        <span class="brand-mark"><svg viewBox="0 0 24 24" fill="none" stroke="#06121a" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 12c-2-2.67-4-4-6-4a4 4 0 1 0 0 8c2 0 4-1.33 6-4Zm0 0c2 2.67 4 4 6 4a4 4 0 0 0 0-8c-2 0-4 1.33-6 4Z"/></svg></span>\n`;
  s += `        <span class="brand-text"><b>${BRAND}</b><span>MLA-C02 · ML + GenAI · Sep 2026</span></span>\n`;
  s += `      </a>\n`;
...
function scripts(p, widgets) {
  let s = "";
  const ws = widgets || [];
  if (ws.indexOf("mermaid-init.js") >= 0) {
    s += `  <script src="https://cdn.jsdelivr.net/npm/mermaid@11/dist/mermaid.min.js"></script>\n`;
  }
  ...
  ws.forEach(function (w) { s += `  <script src="${p}assets/js/${w}"></script>\n`; });
...
function moduleLabelFor(page) {
  ...
  if (page.dir.indexOf("exam") === 0) return "Exam prep";
  if (page.dir.indexOf("interview") === 0) return "Exam prep";
...
function renderPage(page, sections, pages, i) {
  ...
  const desc = BRAND + " — " + page.title + ". A sharp, visual, exam-focused guide to AWS Machine Learning Engineer Associate MLA-C02 — Sep 2026";
  return `<!DOCTYPE html>
...
<head>
  ...
  <link rel="stylesheet" href="${p}assets/css/aws-mla.css" />
...
${mainWrap(page, sections, pages, i)}
      <footer class="footer">
        <span class="fmono">${BRAND} &middot; MLA-C02 &middot; ML + GenAI &middot; Sep 2026</span>
