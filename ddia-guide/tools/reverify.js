...
/* Deep cross-checks beyond verify.js. Run: node tools/reverify.js
   Phase-robust: counts are derived (not hardcoded), so this keeps passing
   as new chapters are appended. Checks gen.js ↔ lessons.js ↔ fragments ↔
   search-index alignment, JSON-config validity, TOC anchors, drift. */
const fs = require("fs");
...
// Registry alignment — self-consistent (no hardcoded totals, so phases pass)
if (genWithLesson.length !== lessons.length)
  fail(`gen.js lesson pages (${genWithLesson.length}) != lessons.js entries (${lessons.length})`);
...
const CONFIGS = ["rf-config", "calc-config", "tr-config", "viz-config", "dv-config"];
...
for (const f of walk(ROOT)) {
  ...
  if (html.includes("data-ddiaviz") && !html.includes("ddia-viz.js")) fail(`${rel}: ddia-viz without script`);
...
// Home — progress dashboard placeholder should reference the live lesson count
const home = fs.readFileSync(path.join(ROOT, "index.html"), "utf8");
if (!home.includes("of " + lessons.length + " lessons"))
  fail(`home: progress dashboard should reference ${lessons.length} lessons`);

// Content signals
let noAnalogy = 0, noQuiz = 0, thin = 0;
const denom = genWithLesson.length || 1;
for (const p of genWithLesson) {
...
console.log("Deep reverify — ddia-guide");
console.log("  verify.js: run separately");
console.log("  gen lesson pages: " + genWithLesson.length + " | lessons: " + lessons.length + " | fragments: " + fs.readdirSync(fragDir).filter((f) => f.endsWith(".html")).length);
...
console.log("  lessons w/o analogy callout: " + noAnalogy + "/" + denom);
console.log("  lessons w/o quiz: " + noQuiz + "/" + denom);
console.log("  lessons <60 lines: " + thin + "/" + denom);
