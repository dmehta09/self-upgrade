#!/usr/bin/env node
/* ============================================================
   Aptitude & Reasoning Field Guide — page assembler (dev-time;
   is plain static HTML with no runtime build / no dependencies).
...
/* target — aim for the right answer, fast */
const ICON = "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%23818cf8' stroke-width='2.2' stroke-linecap='round' stroke-linejoin='round'><circle cx='12' cy='12' r='9'/><circle cx='12' cy='12' r='4.6'/><circle cx='12' cy='12' r='1' fill='%23818cf8' stroke='none'/></svg>";
const BRAND = "Aptitude & Reasoning Field Guide";

function sidebar(p) {
  const groups = M.sidebarGroups();
  let s = `    <aside class="sidebar" id="sidebar">\n`;
  s += `      <a class="brand" href="${p}index.html" aria-label="${BRAND} home">\n`;
  s += `        <span class="brand-mark"><svg viewBox="0 0 24 24" fill="none" stroke="#06121a" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="4.6"/><circle cx="12" cy="12" r="1" fill="#06121a" stroke="none"/></svg></span>\n`;
  s += `        <span class="brand-text"><b>${BRAND}</b><span>Spot &rarr; shortcut &rarr; solve &middot; OA-ready</span></span>\n`;
  s += `      </a>\n`;
...
function scripts(p, widgets) {
  ...
  if (list.indexOf("drill.js") !== -1 && list.indexOf("question-bank.js") === -1) list.splice(list.indexOf("drill.js"), 0, "question-bank.js");
  list.forEach(function (w) { s += `  <script src="${p}assets/js/${w}"></script>\n`; });
...
function stubSections(page) {
  ...
  return `          <section class="hero reveal">
            ...
            <h1>${page.title}</h1>
            <p class="lead">This page of the ${BRAND} is being written &mdash; sharp, visual, exam-ready content is on the way.</p>
          </section>
...
function renderPage(page, sections, pages, i) {
  ...
  const desc = BRAND + " — " + page.title + ". A sharp, visual, exam-ready guide to aptitude & reasoning for tech placements — quantitative aptitude, logical reasoning, verbal ability & data interpretation, with shortcuts and timed practice.";
  return `<!DOCTYPE html>
...
<head>
  ...
  <link rel="stylesheet" href="${p}assets/css/aptitude.css" />
...
${mainWrap(page, sections, pages, i)}
      <footer class="footer">
        <span class="fmono">${BRAND} &middot; quant &middot; reasoning &middot; verbal &middot; data interpretation &middot; OA-ready</span>
