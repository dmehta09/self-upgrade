...
(function () {
  ...
  function applyTheme(t) {
    ...
    try { localStorage.setItem("claude-theme", t); } catch (e) {}
  }
  (function initTheme() {
    var saved;
    try { saved = localStorage.getItem("claude-theme"); } catch (e) {}
  ...
  function highlightShell(code) {
    return esc(code)
      ...
      .replace(/\b(claude|npm|npx|node|pnpm|yarn|pip|pip3|pipx|python|python3|uv|brew|winget|irm|curl|git|gh|cd|ls|cat|export|source|pytest|bash|sh|zsh|docker|mkdir|echo|code|ssh|rm|mv|cp)\b/g, '<span class="tok-bi">$1</span>')
  ...
  function highlightAll() {
    document.querySelectorAll(".codeblock pre code").forEach(function (el) {
      ...
      if (lang === "bash" || lang === "shell" || lang === "console" || lang === "sh") el.innerHTML = highlightShell(raw);
