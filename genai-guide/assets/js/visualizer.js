...
(function () {
  ...
  /* ---------------- heatmap renderer (e.g. attention) ---------------- */
  function mountHeatmap(stage, meta) {
    stage.innerHTML = "";
    var toks = meta.tokens || [];
    var n = toks.length;
    var wrap = el("div", "viz-heat");
    wrap.style.gridTemplateColumns = "auto repeat(" + n + ", minmax(16px, 1fr))";
    wrap.appendChild(el("div", "viz-heat-corner"));
    toks.forEach(function (t) { wrap.appendChild(el("div", "viz-heat-col", t)); });   // key tokens (columns)
    toks.forEach(function (rt, r) {
      wrap.appendChild(el("div", "viz-heat-row", rt));                                // query token (row)
      for (var c = 0; c < n; c++) {
        var cell = el("div", "viz-heat-cell");
  ...
  function paintHeatmap(stage, f) {
    var w = f.weights || [];
    stage.querySelectorAll(".viz-heat-cell").forEach(function (cell) {
      var r = +cell.getAttribute("data-r"), c = +cell.getAttribute("data-c");
      var v = (w[r] && w[r][c] != null) ? w[r][c] : 0;
      cell.style.background = "rgba(var(--accent-rgb)," + (0.05 + 0.95 * v).toFixed(3) + ")";
      cell.classList.toggle("hot", v >= 0.45);
      cell.title = v.toFixed(2);
    });
  }

  /* ---------------- denoise renderer (diffusion) ---------------- */
  function dnHash(x, y) { var n = Math.sin(x * 127.1 + y * 311.7) * 43758.5453; return n - Math.floor(n); }
  function dnBase(x, y, w, h) {                 // an illustrative "sun over ground" scene
    var cx = w * 0.5, cy = h * 0.42, r = w * 0.26, dx = x - cx, dy = y - cy;
    if (y > h * 0.72) return [46, 78, 58];      // ground
    if (dx * dx + dy * dy < r * r) return [129, 140, 248]; // sun (indigo)
    return [11, 16, 32];                        // sky
  }
  function mountDenoise(stage, meta) {
    stage.innerHTML = "";
    var w = meta.w || 28, h = meta.h || 28, scale = meta.scale || 9;
    var cv = el("canvas", "viz-canvas"); cv.width = w; cv.height = h;
    cv.style.width = (w * scale) + "px"; cv.style.height = (h * scale) + "px";
    stage.appendChild(cv);
    stage.__dn = { cv: cv, w: w, h: h };
  }
  function paintDenoise(stage, f) {
    var d = stage.__dn; if (!d) return;
    var ctx = d.cv.getContext("2d"), w = d.w, h = d.h, noise = f.noise != null ? f.noise : 0;
    var img = ctx.createImageData(w, h), data = img.data;
    for (var y = 0; y < h; y++) for (var x = 0; x < w; x++) {
      var b = dnBase(x, y, w, h), g = dnHash(x, y) * 255, i = (y * w + x) * 4;
      data[i] = b[0] * (1 - noise) + g * noise;
      data[i + 1] = b[1] * (1 - noise) + g * noise;
      data[i + 2] = b[2] * (1 - noise) + g * noise;
      data[i + 3] = 255;
    }
  ...
  var Renderers = {
    ...
    heatmap: { mount: mountHeatmap, paint: paintHeatmap },
    denoise: { mount: mountDenoise, paint: paintDenoise }
  };
  ...
  /* Build the stage + caption + player controls if the author didn't hand-write
     them, so a viz can be embedded as just <figure class="viz" data-viz> + config. */
  function ensureScaffold(fig) {
    if (fig.querySelector(".viz-stage")) return;
    function btn(cls, label, txt) { var b = el("button", "viz-btn " + cls, txt); b.setAttribute("type", "button"); b.setAttribute("aria-label", label); return b; }
    fig.appendChild(el("div", "viz-stage"));
    fig.appendChild(el("div", "viz-caption"));
    var controls = el("div", "viz-controls");
    controls.appendChild(btn("viz-reset", "Reset", "⏮"));
    controls.appendChild(btn("viz-prev", "Previous", "‹"));
    controls.appendChild(btn("viz-play", "Play", "▶"));
    controls.appendChild(btn("viz-next", "Next", "›"));
    var prog = el("div", "viz-progress"); prog.appendChild(el("div", "viz-progress-fill")); controls.appendChild(prog);
    controls.appendChild(el("span", "viz-step"));
    var speed = el("span", "viz-speed"), sel = el("select", "viz-speed-sel");
    [["1500", "0.5×"], ["1100", "1×"], ["600", "2×"]].forEach(function (o, i) { var op = el("option", null, o[1]); op.value = o[0]; if (i === 1) op.selected = true; sel.appendChild(op); });
    speed.appendChild(sel); controls.appendChild(speed);
