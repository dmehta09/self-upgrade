#!/usr/bin/env node
/* ============================================================
   Cert Mocks Guide — page assembler (dev-time only; output
...
function sidebar(p) {
  ...
  s += `        <span class="brand-text"><b>${BRAND}</b><span>Mocks · SAA · MLA · CKA · Sep 2026</span></span>\n`;
  s += `      </a>\n`;
...
function renderPage(page, sections, pages, i) {
  ...
  const desc = BRAND + " — " + page.title + ". Full timed mocks for SAA-C03, MLA-C02, and CKA (K8s v1.35) — Sep 2026";
  return `<!DOCTYPE html>
...
<head>
  ...
  <link rel="stylesheet" href="${p}assets/css/cert-mocks.css" />
...
${mainWrap(page, sections, pages, i)}
      <footer class="footer">
        <span class="fmono">${BRAND} &middot; Mocks · SAA · MLA · CKA &middot; Sep 2026</span>
